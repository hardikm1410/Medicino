...
const VoiceService = {
    ...
    init: () => {
        ...
        if (!('webkitSpeechRecognition' in window) && !('SpeechRecognition' in window)) {
            ...
            VoiceService.isSupported = false;
            VoiceService.isInitialized = false;
            
            // Update voice button to show unsupported state
            const voiceBtn = document.getElementById('voiceBtn');
            if (voiceBtn) {
                voiceBtn.disabled = true;
                voiceBtn.title = 'Voice recognition not supported in this browser. Please use Chrome or Edge.';
                voiceBtn.innerHTML = '<i class="fas fa-microphone-slash"></i> Not Supported';
            }
            return;
        }
        ...
        try {
            ...
            VoiceService.recognition.maxAlternatives = 1;
            VoiceService.isSupported = true;
            VoiceService.isInitialized = true;
            ...
            if (voiceBtn) {
                ...
                voiceBtn.innerHTML = '<i class="fas fa-microphone-slash"></i> Error';
            }
        }
    },
    ...
    start: () => {
        ...
        if (!VoiceService.isSupported) {
            Utils.showNotification('Voice recognition is not supported in your browser. Please use Chrome or Edge.', 'error');
            return;
        }
        
        if (!VoiceService.isInitialized || !VoiceService.recognition) {
            Utils.showNotification('Voice recognition not initialized. Please refresh the page.', 'error');
            return;
        }
...
const EventListeners = {
    init: () => {
        ...
        try {
            ...
            // Voice button state management (handled in the main voiceBtn listener above)
            // This is now handled by the main voiceBtn event listener that calls VoiceService.start()
